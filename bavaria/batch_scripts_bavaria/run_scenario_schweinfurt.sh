...
SCENARIO_FILE=bavaria/data/scenario_text_files/schweinfurt/schweinfurt_seed3_hexagon_all.txt
mapfile -t scenarios < "$SCENARIO_FILE"
i=$SLURM_PROCID

# Only process if i is within the block assigned to this job
if [ $i -lt ${START_INDEX:-0} ] || [ $i -gt ${END_INDEX:-$(( ${#scenarios[@]} - 1 ))} ]; then
    exit 0
fi
